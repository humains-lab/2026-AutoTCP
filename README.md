...
The directory [dataset_analysis](/dataset_analysis) includes the following files:

- Test suite runtime distribution: For each SUT, we provide a CSV file and a scatter plot with the total runtime of the test suite executed in each build.
- Test suite runtime differences: A boxplot and a CSV with statistics on the time differences between consecutive test suite runs.
